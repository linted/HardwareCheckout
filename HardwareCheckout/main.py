...
class MainHandler(UserBaseHandler):
    ...
    async def updateQueues(cls):
        ...
        with make_session() as session:
            ...
            cls.queues      = await as_future(session.query(DeviceType.id, DeviceType.name, func.count(UserQueue.userId)).select_from(DeviceType).join(UserQueue, isouter=True).group_by(DeviceType.id, DeviceType.name).filter_by(enabled=1).all)
